...
| A3 | ✅ Closed — Phase 5.5 | AURA 001 became the default runtime (POWER/APPLICATION START → STARTUP → ART). `App.js`'s gate no longer depends on a query parameter, sessionStorage, or `NODE_ENV`; the production root now boots straight into STARTUP → ART, verified against an actual `npm run build` + served production bundle (V0 components — `AuraOrb`, `WidgetOverlay`, `IdleScreen`, `HomePage`, `VisionOverlay` — confirmed absent from the built bundle by string search). The legacy V0 orb cannot mount in the AURA 001 runtime, in dev or production, since the gate remains a true early return. V0 itself is preserved, not deleted, and stays reachable only through the development-only `?v0=1` escape hatch — it is no longer the runtime owner. |
